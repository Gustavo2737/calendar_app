        ...
        if not is_routine:
            ...
            for d_name in dia_map:
                if d_name in recurrence_data.lower():
                    ...
                    # Se o dia já passou na semana, ou é hoje mas a hora já passou, joga pra próxima semana
    ...
    def load_data(self):
        ...
        try:
            ...
            if not rows:
                ...
                empty_card.add_widget(MDLabel(text="Nenhuma atividade cadastrada ainda.", halign="center", theme_text_color="Custom", text_color=(0.7, 0.7, 0.7, 1)))
            ...
            for row in rows:
                ...
                # Mostrando as horas na rotina também
                if is_routine:
                    detalhe_txt = f"Rotina: {recurrence_data if recurrence_data else 'Não especificado'} às {time_str}"
                else:
                    detalhe_txt = f"Data: {due_date} às {time_str}"
                
                timer_txt = self.get_time_left_str(is_routine, due_date, due_time, recurrence_data)
